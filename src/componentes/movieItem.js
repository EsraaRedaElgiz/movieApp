...
const MovieItem = (props) => {
    ...
    return (
        <div className="card h-100">
            <img src={`https://image.tmdb.org/t/p/w500/${movies.poster_path}`} className="card-img-top" alt="..." style={{ height: "300px" }} />
            <div className="card-body">

                <h5 className="card-title" style={{ cursor: "pointer" }} onClick={() => navigateToDetails(movies.id)} >{movies.title}</h5>
                <div className="d-flex align-items-center justify-content-between ">
                    <p  >{movies.release_date}</p>
                    <p  style={{ cursor: "pointer" }} onClick={() => { dispatch(toogleFav(movies)) }}>
                    {favouriteMovies.findIndex(item=>item.id===movies.id)!==-1?
